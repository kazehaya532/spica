...
describe('astronomy formatting', () => {
  ...
  it('keeps essential targets searchable through aliases', () => {
    ...
    expect(SKY_TARGETS.filter((target) => target.kind === 'Star')).toHaveLength(23)
    expect(SKY_TARGETS.find((target) => target.name === 'Albireo')?.aliases).toContain('HIP 95947')
    expect(SKY_TARGETS.find((target) => target.name === 'Winter Albireo')?.aliases).toContain('HIP 35210')
  })
})
