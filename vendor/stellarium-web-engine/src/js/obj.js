...
Module.afterInit(function() {
  ...
  var obj_get_by_hip = Module.cwrap('obj_get_by_hip_js', 'number', ['number']);
  ...
  *    obj.destroy() on each of them.
  ...
  // Return a Hipparcos star directly, loading its small catalog tile on demand.
  Module['getObjByHip'] = function(hip) {
    assert(typeof(hip) == 'number')
    var obj = obj_get_by_hip(hip);
    return obj ? new SweObj(obj) : null;
  };
